...
@Component
public class GetpersonsServiceImpl {
...
public GetpersonsOut getpersonslist(GetpersonsIn in){
	  GetpersonsOut out=new GetpersonsOut();
	  out.setTitle("获取个人信息列表");
	 ...
	 //判断选择的信息是否为空 且是有否有值
 	  ...
 	  if(in.getCard()==null||in.getCard().equals("")){
		...
		try{
		  List<Person> list=dao.getPersons(rowBounds,in);
		  Person person =null;
		...
		for(int i = 0;i<list.size();i++){
		   ...
		   //证件example里面赋值
			...
			if(null!=person.getCredentials())
			 ...
			 out.setCount(dao.getPersonscount(in));
			 out.setDatalist(list);
			 out.setRet("0");
			 out.setMsg("获取个人信息列表成功");
			 return out;
		   }else{
			  out.setRet("0");
			  out.setMsg("无对应的数据");
